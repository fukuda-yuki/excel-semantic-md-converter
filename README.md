...
# TODO: Excel Semantic Markdown Converter

## 0. 前提固定
- [ ] サーバー常駐化しない
- [ ] 各利用者の Windows 端末でローカル実行する
- [ ] 本体は Python CLI ツール
- [ ] 自然言語 UX は GitHub Copilot skill で提供する
- [ ] LLM 実行基盤は GitHub Copilot SDK(local CLI)を使う
- [ ] Excel の見た目取得は Excel COM で画像化する
- [ ] 1 sheet = 1 LLM session を基本にする
- [ ] skill は薄い起動ラッパーに留める
...
- [ ] `README.md` を作成する
- [ ] `src/excel_semantic_md/` 配下のディレクトリを作成する
...
- [ ] `excel-semantic-md resume --job-id <id>` の雛形を作る
- [ ] `--model` オプションを追加する
- [ ] `--vision-model` オプションを追加する
- [ ] `--max-images-per-sheet` オプションを追加する
- [ ] `--save-debug-json` オプションを追加する
- [ ] `--save-render-artifacts` オプションを追加する
- [ ] `--strict` オプションを追加する
...
- [ ] `ChartBlock` を定義する
- [ ] `SheetModel` を定義する
- [ ] 中間表現を Copilot SDK 非依存に保つ

## 5. Excel 構造抽出
- [ ] workbook / worksheet の読み取りを実装する
- [ ] 使用範囲の把握を実装する
- [ ] 結合セル取得を実装する
- [ ] 表候補検出を実装する
- [ ] 段落 / 見出し候補検出を実装する
- [ ] `inspect` で block JSON を出せるようにする

## 6. OOXML 描画メタデータ抽出
- [ ] drawing 情報の読み取りを実装する
- [ ] image 参照取得を実装する
- [ ] shape 参照取得を実装する
- [ ] chart 参照取得を実装する
- [ ] anchor / 位置情報取得を実装する
...
- [ ] 表と画像の紐付けを実装する
- [ ] 表とグラフの紐付けを実装する
- [ ] 孤立オブジェクトを独立 block として扱えるようにする

## 8. Excel COM レンダリング
- [ ] Excel session ラッパーを実装する
- [ ] 読み取り専用で開く処理を実装する
- [ ] `Range` の画像化を実装する
- [ ] `Shape` の画像化を実装する
- [ ] `Chart` の PNG 出力を実装する
- [ ] 例外時も Excel プロセスを確実に終了する
- [ ] `render` コマンドで個別出力できるようにする

## 9. Copilot SDK 接続
- [ ] local CLI 前提の client 初期化を実装する
- [ ] sheet 単位 session 作成を実装する
- [ ] prompt builder を実装する
- [ ] attachment builder を実装する
- [ ] response parser を実装する
- [ ] hooks の最小版を実装する
...
- [ ] block JSON を含める
- [ ] asset 参照を含める
- [ ] style / targetFormat 指示を含める
- [ ] 応答に `sheet_summary` を含める
- [ ] 応答に `sections` を含める
- [ ] 応答に `figures` を含める
- [ ] 応答に `unknowns` を含める
- [ ] 応答に `markdown` を含める
...
## 13. Orchestrator
- [ ] workbook 全体の変換フローを実装する
- [ ] sheet ごとの順次処理を実装する
- [ ] 失敗時の中間成果物保存を実装する
- [ ] `convert` で E2E 実行できるようにする
...
- [ ] 必要環境を書く
- [ ] Excel 必須であることを書く
- [ ] Copilot CLI ログイン前提を書く
- [ ] 基本コマンド例を書く
- [ ] skill の配置方法を書く
- [ ] 制限事項を書く

## 16. テスト
- [ ] table only workbook fixture を作る
- [ ] table + shape text workbook fixture を作る
- [ ] table + chart workbook fixture を作る
- [ ] table + image workbook fixture を作る
- [ ] multi-sheet workbook fixture を作る
- [ ] block 検出結果のテストを書く
- [ ] asset 出力結果のテストを書く
- [ ] manifest 整合性のテストを書く
- [ ] Markdown に主要要素が含まれることを確認する

## 17. 受け入れ条件
- [ ] `.xlsx` を入力して `result.md` を生成できる
- [ ] 複数 sheet を順に処理できる
- [ ] 表が Markdown table として出力される
- [ ] テキスト入り図形が注記として反映される
- [ ] chart を画像付きセクションとして扱える
- [ ] 画像を attachment として LLM に渡せる
- [ ] `manifest.json` で block と asset の対応が追える
- [ ] CLI 実行で再現できる
- [ ] skill から起動しても同じ CLI を使う
- [ ] 失敗時に Excel プロセスが残りにくい
...
- [ ] Phase 3: drawing metadata
- [ ] Phase 4: rendering
- [ ] Phase 5: Copilot SDK
- [ ] Phase 6: output composition
- [ ] Phase 7: skill

## 20. 一文サマリ
- [ ] 本体は Python CLI、知能は内部 Copilot SDK、skill は薄い起動ラッパー、Excel の見た目取得はローカル Excel COM
