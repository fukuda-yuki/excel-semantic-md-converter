...
Phase 1 のプロダクト仕様・実装方針・優先順位の正はこの文書とする。
...
```bash
excel-semantic-md setup [--out "C:\out"]
```
...
- Copilot CLI が利用できる可能性があること。候補は `copilot` と `gh copilot` の順に確認する。
- `gh copilot` を利用できる場合、Copilot CLI のサインイン状態を確認できること。確認できない場合は案内に留める。
- skill launcher の配置先または利用方法を案内できること。
- `--out` で出力先に使うディレクトリをユーザーが指定した場合、作成可能性と書き込み可能性を確認できること。

`setup --out` は確認用の一時ファイルを作成して削除する。確認のために作成した空ディレクトリは可能な範囲で削除する。
...
`phase1-ooxml-visual-metadata` では各 sheet に `visuals` 配列を追加し、drawing 読み取り失敗など sheet 単位の補足情報は `warnings` に追加する。
`phase1-visual-linking` では `blocks` を post-linking 状態にし、visual 由来 block と block 間リンク情報を `inspect` で確認できるようにする。
...
- 数式セルに Excel 保存済みの表示値キャッシュが無い場合、その sheet は failed として扱う。
...
- `visual_id`: visual 由来 block の元 visual ID。cell-based block では `null`。
- `related_block_id`: 関連先 block ID。visual 由来 block が table / paragraph / heading に紐付いた場合はその block ID、独立 block や cell-based block では `null`。
...
linked / unlinked を問わず block 化し、`inspect` の `blocks` は post-linking の最終状態を返す。
visual 由来 block は `visual_id` を持ち、紐付いた場合は `related_block_id` に関連先 block ID を入れる。
`unknown` visual はこの milestone では block 化せず、`visuals` と warning に残す。
...
heading の説明範囲は、その heading block から次の heading block 手前までとする。
`oneCellAnchor` は 1x1 cell rect、`twoCellAnchor` は from / to を含む rect として扱う。
`absoluteAnchor` など cell rect を作れない visual は warning を付けた独立 block とし、sheet 末尾側に synthetic anchor を割り当てる。
...
`inspect` の visual metadata は各 sheet の `visuals` 配列へ出力する。
`visuals` は要素がない場合でも `[]` を出力する。

各 visual 要素は次の共通フィールドを持つ。

- `id`: `s{sheet_index:03d}-v{visual_index:03d}-{kind}`
- `kind`: `shape` / `image` / `chart` / `unknown`
- `anchor`: `anchor_type`、`from`、`to`、`a1`
- `source`: `drawing_part`、`relationship_id`、`target_part`
...
- `image`: `alt_text`
- `chart`: `title`、`series`
- `unknown`: `unknown_kind`、`description`

`chart.series[]` は次のフィールドを持つ。
...
- OOXML の `oneCellAnchor` / `twoCellAnchor` / `absoluteAnchor` を `anchor_type` にそのまま出す。
- `from` / `to` の `row` / `col` は 1-based で表現する。
- `row_offset_emu` / `col_offset_emu` は存在する場合のみ出す。
- `a1` は cell-based に表現できる場合だけ出し、`absoluteAnchor` では出さない。

### 6.2 対象外要素

SmartArt、OLE、group shape などを完全解釈できない場合は、`unknown` visual または warning として `inspect` / `manifest.json` に残す。
可能ならレンダリング画像だけを生成し、LLM に補助情報として渡す。

drawing part 自体の parse failure、drawing relationship target 不足、chart part 不足などは workbook 全体を失敗にせず、該当 sheet の `warnings` または visual の `warnings` に残す。
ただし workbook / worksheet の主要 XML 破損は従来どおり CLI error とする。
...
`phase1-ooxml-visual-metadata` では最終 asset path はまだ確定しない。
この milestone では `asset_candidate.source_part`、`extension`、`content_type` を `inspect` に出し、block id を使う最終命名は後続 milestone で確定する。
